 ...
 * @example
 * PolymerVis.insertCssIntoShadowRoot('https://some.css', ele.shadowRoot,
 * 'custom');
 */
PolymerVis.insertCssIntoShadowRoot = function insertCssIntoShadowRoot(
    cssSrc, shadowRoot, onload, id = 'custom') {
  ...
  httpRequest.onreadystatechange = () => {
    if (httpRequest.readyState === XMLHttpRequest.DONE) {
      if (httpRequest.status === 200) {
        var style = shadowRoot.querySelector(`#${id}`) ||
...
/**
 * Check if the node is inside a
 * [`shadowRoot`](https://developer.mozilla.org/en-US/docs/Web/API/shadowRoot)
 * or not. Return the `shadowRoot` otherwise return `false`.
